...
describe( '<wpd-menu> + <wpd-menu-item>', () => {
	...
	test( 'menu has role=menu, item defaults to role=menuitem', async () => {
		host.innerHTML = `
			<wpd-menu>
				<wpd-menu-item value="a">A</wpd-menu-item>
			</wpd-menu>
		`;
		await tick();
		const menu = host.querySelector( 'wpd-menu' )!;
		const item = host.querySelector( 'wpd-menu-item' )!;
	...
	test( 'checkbox variant paints the check + reflects aria-checked', async () => {
		host.innerHTML = `
			<wpd-menu-item role="menuitemcheckbox" value="startup" checked>
				Open on startup
			</wpd-menu-item>
		`;
		await tick();
	...
	test( 'click emits wpd-menu-item-click with value', async () => {
		host.innerHTML = `
			<wpd-menu-item value="open-another">Open another</wpd-menu-item>
		`;
		await tick();
		const item = host.querySelector( 'wpd-menu-item' )!;
		let heard: string | null = null;
		item.addEventListener( 'wpd-menu-item-click', ( e ) => {
	...
	test( 'icon attribute surfaces a dashicon span', async () => {
		host.innerHTML = `
			<wpd-menu-item icon="dashicons-plus-alt2" value="x">Add</wpd-menu-item>
		`;
		await tick();
		const item = host.querySelector( 'wpd-menu-item' )!;
