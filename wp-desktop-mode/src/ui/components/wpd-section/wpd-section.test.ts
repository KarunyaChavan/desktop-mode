/**
 * `<wpd-section>` — smoke test. Verifies that heading +
...
describe( '<wpd-section>', () => {
	...
	test( 'renders heading + description + slotted content', async () => {
		host.innerHTML = `
			<wpd-section heading="Wallpaper" description="Backdrop">
				<span class="child">body</span>
			</wpd-section>
		`;
		await tick();
		const section = host.querySelector( 'wpd-section' )!;
